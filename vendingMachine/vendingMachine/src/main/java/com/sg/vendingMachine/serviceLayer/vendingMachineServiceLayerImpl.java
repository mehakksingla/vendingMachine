...
public class vendingMachineServiceLayerImpl implements vendingMachineServiceLayer{
    ...
    //Check Business rules here: negative values, too large values
    if(money.compareTo(new BigDecimal("5")) > 0){
        auditDao.writeAuditEntry("USER INSERTED TOO MUCH MONEY, PETTY CASH EXCEPTION THROWN, FUNDS REJECTED");
    ...
    public Change vendItem(String choice) throws InsufficientFundsException, ZeroInventoryException, ItemPersistenceException {
        ...
        if(dao.returnInsertedMoney().compareTo(dao.getItem(choice).getPrice()) < 0){
            ...
            throw new ZeroInventoryException("No stock for requested item\n");
        }
        else{
            dao.vendItem(choice);
